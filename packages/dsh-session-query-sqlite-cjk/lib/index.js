...
var CjkSessionQueryEngine = class extends SessionQueryEngine {
	...
	/**
	 * Observe one exact live or prepared Session without a persistence
	 * listing preflight. Explicitly defined on the CJK concrete class so the
	 * service contract stays satisfied even when this build inherits from an
	 * older `@deepseek-ai/dsh-session-query` base that predates `observeSession`
	 * (issue #1: Desktop 2.0.4's session-controller calls it on a cold session
	 * and the rc.7 base has no such method). Delegates to the inherited
	 * `SessionObservationReader` when present; otherwise synthesizes a live
	 * observation lease directly from the session store + persistence service.
	 * @param sessionId - logical Session identity.
	 * @param options - `{ signal?, projectionMode? }`.
	 * @returns a caller-owned observation lease (`[Symbol.dispose]`).
	 */
	observeSession(sessionId, options = {}) {
		const baseRead = this._observations && typeof this._observations.read === "function" ? this._observations.read.bind(this._observations) : void 0;
		if (baseRead !== void 0) return baseRead(sessionId, options);
		return observeSessionFallback(this, sessionId, options);
	}
...
/**
 * Fallback for the `ctx.sessionQuery.observeSession` service contract when the
 * inherited `SessionQueryEngine` base predates `observeSession` (rc.7).
 * Builds the same caller-owned lease the new base's `SessionObservationReader`
 * would return: live-preferred, with an unpinned prepared cold read for
 * persisted sessions. `inheritedEventCount` and `cursor` mirror the new
 * observation shape so consumers (`session-controller`) can read them.
 */
async function observeSessionFallback(engine, sessionId, options) {
	const { signal, projectionMode = "all" } = options ?? {};
	const sessions = engine.ctx.sessions;
	const live = sessions.get(sessionId);
	if (live !== void 0) {
		const seq = live.seq ?? live.events.length;
		const materialize = () => live.events.slice(0, seq).map((event) => structuredClone(event));
		const projections = projectionMode === "all" ? engine.ctx.get("sessionProjections")?.snapshot(live) : void 0;
		return observationLease({
			source: "live",
			header: live.header,
			inheritedEventCount: live.inheritedEventCount ?? 0,
			events: materialize,
			cursor: seq === 0 ? -1 : seq - 1,
			...projections === void 0 ? {} : { projections }
		});
	}
	const persistence = engine.ctx.get("sessionPersistence");
	if (persistence === void 0) throw new SessionQueryError(`session "${sessionId}" not found`, "SESSION_QUERY_SESSION_NOT_FOUND");
	const stat = await persistence.stat(sessionId, signal === void 0 ? void 0 : { signal });
	if (stat === void 0) throw new SessionQueryError(`session "${sessionId}" not found`, "SESSION_QUERY_SESSION_NOT_FOUND");
	const loaded = await persistence.inspect(sessionId, signal === void 0 ? void 0 : { signal });
	const attached = sessions.get(sessionId);
	if (attached !== void 0) return observeSessionFallback(engine, sessionId, options);
	const events = loaded.events.map((event) => structuredClone(event));
	const projections = projectionMode === "all" ? engine.ctx.get("sessionProjections")?.snapshotFrom?.(loaded.header, events) : void 0;
	return observationLease({
		source: "prepared",
		header: loaded.meta,
...
function observationLease(observe) {
	let disposed = false;
	const events0 = observe.events;
	return {
		get source() { return observe.source; },
		get header() { return observe.header; },
		get inheritedEventCount() { return observe.inheritedEventCount; },
		get events() { return events0(); },
		get cursor() { return observe.cursor; },
		...observe.revision === void 0 ? {} : { get revision() { return observe.revision; } },
		...observe.projections === void 0 ? {} : { get projections() { return observe.projections; } },
		retain: () => {
			if (disposed) throw new Error("session observation is disposed");
			return observationLease(observe);
		},
